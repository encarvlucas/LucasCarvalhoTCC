...
Pode-se remover as componentes da velocidade do fluido $v_{f,x}$ e $v_{f,y}$ da equação \refeq{temp1}, pois serão utilizados os componentes da velocidade no último passo de tempo para estes valores, tornando-se a equação linear.
...
Este é o caso da solução da equação de Navier-Stokes.
...
\end{equation}
onde $dt$ é um passo de tempo infinitamente pequeno.

Será utilizada a notação de índice de variáveis no tempo como sobrescrito, onde $t_n$ é o índice do passo no tempo, $\omega^{t_{n+1}} = \omega(t_n + dt)$ e $\omega^{t_{n}} = \omega(t_n)$.
Então a discretização final do sistema corrente-vorticidade é:
\begin{equation}
    \left(
        \dfrac{\mathbf{M}}{\Delta t} +
        v_{f,x}^{t_{n}}
        \mathbf{G_x}+
        v_{f,y}^{t_{n}}
        \mathbf{G_y} +
