...
\label{sec_cons_qmov}
A conservação de quantidade de movimento é similar a conservação de massa, porém, é uma equação vetorial.
...
Substituindo a definição de $\boldsymbol{\tau}$ na \refeq{cons_qmov_tensor}, obtém-se:
\begin{equation}
    \rho_f \left(
    	\dfrac{\partial \vec{v}_f}{\partial t} + 
    	\vec{v}_f.\vec{\nabla}\vec{v}_f
...
Dividindo-se todos os termos pela massa específica pode-se reescrever a equação \refeq{tau_simple}, resultando-se na forma simplificada da \textit{Equação de Navier-Stokes} para fluidos newtonianos incompressíveis e com viscosidade constante:
\begin{equation}
	\dfrac{\partial \vec{v}_f}{\partial t} + 
	\vec{v}_f.\vec{\nabla}\vec{v}_f =
    -\dfrac{1}{\rho_f} \vec{\nabla}p +
...
\label{sec_eq_part}
Neste trabalho as partículas são tratadas de maneira pontual, isto é, são tomadas como esferas sólidas perfeitas. 
...
As forças aplicadas às partículas foram determinadas a partir da equação de \textbf{Basset–Boussinesq–Oseen} (BBO), que descreve as forças sofridas por partículas sob efeito de escoamentos em estado laminar.
...
\begin{itemize}
    ...
    \item \textbf{Força de Arrasto}:
        ...
        Este termo causa o efeito de trazer a velocidade da partícula à velocidade do fluido já que torna a força proporcionalmente maior quanto mais afastadas elas se encontrarem.
    ...
    \item \textbf{Força de Sustentação}:
        A força de sustentação é uma componente que tende a mover um corpo quando existe um gradiente de velocidades do escoamento entre o seu contorno.
        A geometria do corpo pode causar um diferencial entre a velocidade do fluido ao seu redor, como ocorre na asa de um avião e este efeito que causa a força exercida para elevação do avião.
