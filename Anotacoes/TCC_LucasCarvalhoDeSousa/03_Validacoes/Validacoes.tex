...
O erro relativo médio calculado para este caso foi de $1.4251\%$ e com desvio padrão de $0.2793\%$.
\begin{figure}[H]
    \centering
...
Na simulação das forças, foram aplicadas cada uma separadamente a uma partícula isolada para se avaliar seus efeitos e comparar sua solução numérica com a solução analítica respectiva.
Para a força gravitacional, utilizou-se uma malha com as condições apresentadas na \ref{grav_bc}.
A velocidade inicial tomada foi $v_{y0}=0$.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/forces_boundary_conditions.pdf}
    \caption{Condições de contorno de uma partícula sob efeito da força gravitacional em um escoamento.}
    \label{grav_bc}
\end{figure}

Na \ref{grav_comp} pode-se observar a evolução da posição da partícula no eixo $y$ em função do tempo.
Foi utilizada uma malha triangular linear não estruturada com 2304 elementos e 417 nós, seu campo de velocidades não foi calculado e foi criado com um valor constante $U=2m/s$.
O erro relativo médio calculado para este caso foi de $7.706e^{-4}\%$ e com desvio padrão de $1.753e^{-5}\%$.
\begin{figure}[H]
    \centering
